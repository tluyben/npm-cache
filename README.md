...
To use a custom domain (e.g., `registry.forge.local`):

1. Add to `/etc/hosts`:
   ```
   127.0.0.1 registry.forge.local
   ```

2. Update `config.yaml`:
   ```yaml
   listen: 0.0.0.0:80
   url_prefix: https://registry.forge.local/
   ```

3. Configure NPM:
   ```bash
   export NPM_CONFIG_REGISTRY=https://registry.forge.local/
   ```

Note: For HTTPS, you'll need to configure SSL certificates in Verdaccio.
